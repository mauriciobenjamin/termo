...
def extract_thermal_data_fast(img_rgb, temp_min, temp_max):
    ...
    temp_matrix = temp_min + (norm_values.reshape(rows, cols) * (temp_max - temp_min))
...
def get_aligned_blend(visible_img, thermal_data_matrix, src_pts, dst_pts, alpha, cmap_name, t_range):
    ...
    aligned_thermal_data = cv2.warpPerspective(thermal_data_matrix, h_matrix, (w_vis, h_vis))
...
def file_uploaders():
    ...
    upload_vis = mo.ui.file(label="Sube Imagen Visible (TIFF/JPG)", kind="image")
...
def load_images(upload_vis, upload_therm):
    ...
    if upload_vis.value:
        vis_img = cv2.cvtColor(cv2.imdecode(np.frombuffer(upload_vis.value[0].contents, np.uint8), 1), cv2.COLOR_BGR2RGB)
    
    if upload_therm.value:
        therm_img_raw = cv2.cvtColor(cv2.imdecode(np.frombuffer(upload_therm.value[0].contents, np.uint8), 1), cv2.COLOR_BGR2RGB)
...
def thermal_params(therm_img_raw):
    ...
    c1, c2 = mo.columns(2)
    with c1:
        t_min = mo.ui.number(start=-50, stop=500, value=28.5, label="Temp Mín Foto (°C)")
    with c2:
        t_max = mo.ui.number(start=-50, stop=500, value=60.0, label="Temp Máx Foto (°C)")
    
    if therm_img_raw is None:
        return t_min, t_max, None
        
    temp_data_matrix, _ = extract_thermal_data_fast(therm_img_raw, t_min.value, t_max.value)
...
def rendering_controls():
    ...
    with c1:
        alpha = mo.ui.slider(start=0, stop=1, step=0.05, value=0.5, label="Opacidad Térmica")
    with c2:
        cmap = mo.ui.dropdown(options=['inferno', 'magma', 'jet', 'plasma', 'viridis'], value='inferno', label="Mapa de Color")
    with c3:
