...
def _():
    ...
    return KDTree, cv2, mo, np, plt


@app.cell
def extract_thermal_data_fast(KDTree, cv2, np, plt):
    ...
    def get_aligned_blend(visible_img, thermal_data_matrix, src_pts, dst_pts, alpha, cmap_name, t_range):
        ...
        src = np.array([[p['x'], p['y']]
                       for p in src_pts[:4]], dtype=np.float32)
        dst = np.array([[p['x'], p['y']]
                       for p in dst_pts[:4]], dtype=np.float32)
        ...
        t_min, t_max = t_range
        t_norm = np.clip((aligned_thermal_data - t_min) /
                         (t_max - t_min), 0, 1)
        ...
        blended = cv2.addWeighted(
            visible_img, 1-alpha, thermal_colored, alpha, 0)
        return blended
...
@app.cell
def file_uploaders(mo):
    _head = mo.md("### 1. Cargar Archivos")
    upload = mo.ui.file(filetypes=[".tif", ".jpg", ".bmp", ".png"], label="Sube Archivo", kind="area")
    ...
    mo.vstack([_head, upload])
    return


@app.cell
def load_images(cv2, np, upload_therm, upload_vis):
    vis_img = None
    therm_img_raw = None
